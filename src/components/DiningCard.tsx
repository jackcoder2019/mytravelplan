      ...
      <div className="px-5 pb-5 space-y-3">
        {dining.map(d => (
          <div key={d.id} className="border border-navy-light rounded-xl p-3 space-y-2 group relative">
            <button onClick={() => remove(d.id)} className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-gray-500 hover:text-accent-red transition">×</button>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-xs text-gray-400">Restaurant</label>
                <input className="editable text-sm mt-0.5" value={d.name} onChange={e => update(d.id, { name: e.target.value })} placeholder="Name" />
              </div>
              <div>
                ...
                <select className="w-full bg-transparent border-b border-gray-600 text-sm mt-0.5 outline-none focus:border-accent-teal"
                  value={d.meal} onChange={e => update(d.id, { meal: e.target.value })}>
                  {MEALS.map(m => <option key={m} value={m} className="bg-navy-mid">{m}</option>)}
                </select>
              </div>
            </div>
            <input className="editable text-sm" value={d.address} onChange={e => update(d.id, { address: e.target.value })} placeholder="Address" />
            <div className="grid grid-cols-2 gap-2">
              <input className="editable text-sm" value={d.hours} onChange={e => update(d.id, { hours: e.target.value })} placeholder="Hours" />
              <input className="editable text-sm" value={d.notes} onChange={e => update(d.id, { notes: e.target.value })} placeholder="Notes / reservation" />
            </div>
          </div>
        ))}
